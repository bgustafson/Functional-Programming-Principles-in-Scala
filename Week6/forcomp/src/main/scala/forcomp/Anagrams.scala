...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    if (s.isEmpty) Nil
    else (s flatMap (w => wordOccurrences(w)) groupBy(_._1) map (pair => (pair._1, pair._2.map(_._2).sum)) toList) sorted
  }
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = ((((x ++ (y map (li => (li._1, -li._2)))) groupBy(x => x._1)) mapValues(_.map(_._2).sum) toList) filter (_._2 != 0)) sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def getAnas(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(Nil)
      else for (
        comb <- combinations(occurrences) if dictionaryByOccurrences.keySet(comb);
        anas <- dictionaryByOccurrences(comb);
        sentences <- getAnas(subtract(occurrences, comb))
      ) yield { anas :: sentences }

    getAnas(sentenceOccurrences(sentence))
  }
}
